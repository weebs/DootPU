...
type Shader =
    ...
    // static member createFragment (shader: 'a -> Quotations.Expr<_>) : Fragment_<'a> =
        // { value = Unchecked.defaultof<_> }
    static member createFragment (shader: Quotations.Expr<'a -> _>) : Fragment_<'a> =
        { value = Unchecked.defaultof<_> }
...
type Module = {
    ...
    static member empty = { unfinishedBindings = Map.empty; bindings = Map.empty; structs = Map.empty; fns = Map.empty }
module rec Print =
    open Wgsl
    ...
    let shader (compiled: WgslModuleStatement list) =
        ...
        for item in compiled do
            ...
            | ModuleVar(binding, name, types, wgslType) ->
                let bindingStr =
                    match binding with
                    | Some (group, binding) -> $"@group({group}) @binding({binding}) "
                    | None -> ""
                let varTypes =
                    if types = [] then "" 
                    else
                ...
                items.Add [
                    bindingStr + "var" + varTypes + name + ": " + type_ + ";"
                ]
            | Struct (name, fields) ->
                let strStructFields =
                    fields 
    ...
    let module' (module_: Module) =
        ...
        shader items
open Wgsl
let rec function' (expr: Quotations.Expr)
    // : (string * WgslType) list * WgslStatement list * WgslType
    =
    let rec parse acc = function
        | Patterns.Call (_, method, [ Patterns.Value (o, _); e ])
            when method.Name = "Location" ->
...
// and translateModule (module_: Quotations.Expr<Fragment<'t -> 'u>>) =
    ...
    | Patterns.Let (var, Patterns.Call (_, method, [ varTypes; Patterns.Var v ]), e)
        when method.Name = "Var" ->
        let binding = module_.unfinishedBindings[v.Name]
        // { module_ with bindings }
        { module_ with
            bindings = module_.bindings.Add(var.Name, {
                name = var.Name
    ...
    | Patterns.Let (v, Patterns.Call (None, method, [ l ]), following)
        when method.Name = "VertexShader" || method.Name = "FragmentShader" ->
        let args, abs, t' = function' l
...
and translateModule (expr: Quotations.Expr) (module_: Module) : Module =
    let rec getModule (e: Quotations.Expr) acc =
        ...
        | Patterns.Let (var, Patterns.TupleGet (value, index), expr) ->
            (acc @ [ var, value.Type.GenericTypeArguments[index] ]) |> getModule expr
        | _ -> e, acc
    match expr with
    | Patterns.Lambda (var, e) -> 
        ...
        let newStructs = (module_.structs, structs @ moreStructs |> List.distinct) ||> List.fold (fun acc struct_ -> acc.Add(fst struct_, struct_))
            // |> Map.ofList
        let bindings = [
...
and visitExpr (fn: Quotations.Expr -> Quotations.Expr) (e: Quotations.Expr) =
    match e with
    ...
    | Patterns.IfThenElse (cond, whenTrue, whenFalse) -> E.IfThenElse (fn cond, fn whenTrue, fn whenFalse)
    | Patterns.Let (var, value, e) -> E.Let (var, fn value, fn e)
    | expr -> expr
and visitInner f e =
    (visitExpr (visitInner f) e) |> f
...
and (|WgslStruct|_|) (t: Type) : WgslStruct option =
    if FSharpType.IsRecord t && not t.IsGenericType then
        let fields = FSharpType.GetRecordFields t
        let result = fields |> Array.map (fun f ->
            let attributes =
                f.CustomAttributes |> Seq.toArray |> Array.choose (fun data ->
...
and structsUsed (e: Quotations.Expr) =
    ...
    | Patterns.Call (thisArg, method, _) ->
        let types =
            ...
            |> Array.append [| if thisArg <> None then thisArg.Value.Type |]

...
// and translateModule (module_: Quotations.Expr<Fragment<'t -> 'u>>) =
    ...
    | Patterns.Let (v, Patterns.Call (None, method, [ l ]), following)
        ...
        Function (v.Name, args, [ if method.Name = "VertexShader" then "vertex" else "fragment" ], Some rt, addReturn abs) :: translateModule following
    | _ -> []
and toType (t: System.Type) =
    ...
    // | t when t.IsGenericType && t.GetGenericTypeDefinition() = typedefof<vec2<_>> ->
        // Wgsl.DefinedType 
    | t when t = typeof<vec2<float32>> -> WgslType.DefinedType "vec2f"
...
and shader' (screen: Screen, circles: float32[]) = <@
    ...
    //     let direction = normalize(point - cameraOrigin)
...
// type Struct = {
//     [<BuiltIn(Builtin.Position); Location 0>]
//     foo: vec4<float32>
// }

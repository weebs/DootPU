module Raycast.Renderer.SimpleVoxelRender
...
type Shader(data: int[], output: int[]) =
    let sdfSphere s p r = length(s - p) - r
    ...
    let sdfPt (pos: vec3f) =
        ...
        while i <= 200 do
            ...
            let s =
                vec3(float32 i, float32 i, float32 i) * 0.2f
                + vec3(0f, 0f, 4f)

            let dist = sdfSphere s pos r
            d <- min(d, dist)
            i <- i + 1

        d

    let march (pos: vec3f) (normalDir: vec3f) =
        let mutable p = pos
        let mutable dist = sdfPt pos
        let mutable i = 0

        while dist > 0.02f && dist < 100f && i < 100 do
            i <- i + 1
            p <- p + (dist * normalDir)
            dist <- sdfPt p

        if dist <= 0.02f then vec4(0f, 0.7f, 1f, 1f) else vec4(0f)
    ...
    member this.fragment(vertexOutput: Output) =
        ...
        let pos = vec3(vertexOutput.xy.x, vertexOutput.xy.y, 0f)
        let normalDir = normalize(vec3(pos.x, pos.y, 1f))
        march pos normalDir
...
let init (window: IWindow) =
    // let gridSize = 200
    let mapVoxels = Array.zeroCreate(gridSize * gridSize * gridSize)
    // let r = Random()
    // for i in 1..10000 do
    //     mapVoxels[r.NextInt64 mapVoxels.Length |> int] <-
    //         Wgsl.vec4(r.NextSingle(), r.NextSingle(), r.NextSingle(), 1f)
    mapVoxels[0 + (11 * gridSize) + (8 * gridSize * gridSize)] <- Wgsl.vec4(0f, 1f, 1f, 1f)
    mapVoxels[8 + (11 * gridSize) + (8 * gridSize * gridSize)] <- Wgsl.vec4(0f, 1f, 1f, 1f)
    mapVoxels[4 + (15 * gridSize) + (8 * gridSize * gridSize)] <- Wgsl.vec4(0f, 1f, 1f, 1f)
    mapVoxels[4 + (7 * gridSize) + (8 * gridSize * gridSize)] <- Wgsl.vec4(0f, 1f, 1f, 1f)
    let wgpu = new WebGPU'(window)
    let state = wgpu.CreateBinder WebGPU.Raymarching.VertexFragmentShader
    let (camera, state) = Wgpu.Bind state
    let (bGridSize, state) = Wgpu.Bind state
    let (voxels, state) = Wgpu.Bind state mapVoxels.Length
    ...
    win.onRender (fun view t ->
        ...
        if not wroteMap then
            bGridSize.Write(wgpu, queue, gridSize)
            voxels.Write(wgpu, queue, 0uL, mapVoxels)
            wroteMap <- true

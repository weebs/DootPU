...
module Shaders =
    ...
    type Raycaster
        (cfg: Config, grid: int[], output: RaycastResult[], steps: Steps[])
        =
        ...
        let dda (rayPos: vec3f) (rayDir1: vec3f) =
            ...
            let sideDist =
                (sign(rayDir) * (mapPos - rayPos)
                 + (sign(rayDir) * 0.5f)
                 + 0.5f)
                * deltaDist

            let mask =
                lessThanEqual(sideDist, min(sideDist.yzx, sideDist.zxy))

            let maskf =
                vec3(float32(mask.x), float32(mask.y), float32(mask.z))

            let correctedVector1 = sideDist * fixedDistance * maskf

            let correctedVector =
                sideDist * fixedDistance * maskf * deltaDist

            let c = sideDist * maskf

            let correctDistance1 =
                max(
                    abs(correctedVector.x),
                    max(abs(correctedVector.y), abs(correctedVector.z))
                )
            ...
            let distance =
                max(
                    abs(pointDistances.x),
                    max(abs(pointDistances.y), abs(pointDistances.z))
                )
            ...
            let toReturn2 =
                rayPos + scaledOffset + (rayStep * epsilon * maskf)
            // toReturn
        ...
        let toIndex (voxel: vec3f) =
            int voxel.x
            + int voxel.y * cfg.gridSize
            + int voxel.z * cfg.gridSize * cfg.gridSize
        ...
        let outOfBounds (pos: vec3f) =
            let q = abs(floor pos)

            int q.x >= cfg.gridSize
            || int q.y >= cfg.gridSize
            || int q.z >= cfg.gridSize
        ...
        member this.raycast(pixelX, pixelY) =
            ...
            // let offsetX = (pcX - 0.5f) * screenWidthMeters
            // let offsetY = (pcY + 0.5f) * screenWidthMeters
            let voxelScaling = 32f //88f
            let f = 1f / voxelScaling

            let dirY =
                ((float32 cfg.heightPixels * 0.5f) - float32 pixelY)
                / float32 cfg.heightPixels
                // / voxelScaling

            let dirX =
                (float32 pixelX - (float32 cfg.widthPixels / 2f))
                / float32 cfg.widthPixels
                // / voxelScaling
            ...
            // if index < maxIndex && grid[index] <> 0 then
                // rayhit <- grid[index]
            if checkPos pos <> 0 then
                rayhit <- checkPos pos

            let mutable i = 0
            let dir = vec3(dirX, dirY, f)
            let maxIterations = 400

            if pixelX = cfg.debugX && pixelY = cfg.debugY then
                logStep(0, pos, voxel)

            while rayhit = -1 && i < maxIterations do
                i <- i + 1
        ...
        member this.main
            ([<BuiltIn(Builtin'.global_invocation_id)>] globalId:
                vec3<uint>)
            =
